...
const Library: React.FC<LibraryProps> = ({ 
  ...
  const groupedBooks = useMemo(() => {
    ...
    // Sort classes within each section
  ...
  const handleDeleteSection = () => {
    if (sectionToDelete) {
        ...
        if (booksInSection) {
            alert("এই শাখায় বই থাকায় এটি মুছে ফেলা যাবে না। প্রথমে বইগুলো মুছুন বা অন্য শাখায় স্থানান্তর করুন।");
      ...
      <div className="space-y-4">
                ...
                <div className={`transition-[max-height,opacity] duration-500 ease-in-out overflow-hidden ${isExpanded ? 'max-h-screen opacity-100' : 'max-h-0 opacity-0'}`}>
                    <div className="p-4">
                        {groupedBooks[section.id] ? (
                            Object.keys(groupedBooks[section.id]).map(className => (
                                <div key={className} className="mb-6 last:mb-0">
                                    ...
                                    <table className="min-w-full">
                                        <thead>
                                            <tr>
                                                <th className="py-2 px-3 text-left text-xs font-medium text-gray-500 uppercase">বইয়ের নাম</th>
                                                ...
                                                <tr key={book.id}>
                                                    ...
                                                    <td className="py-2 px-3 whitespace-nowrap">
                                                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${book.isAvailable ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                                            {book.isAvailable ? 'উপলব্ধ' : 'ইস্যু করা হয়েছে'}
                                                        </span>
                                                    </td>
                                                    {hasPermission('লাইব্রেরি') && (
                            ...
                            <p className="text-center text-gray-500 py-4">এই শাখায় কোনো বই যোগ করা হয়নি।</p>
                        )}
                    </div>
                </div>
            </div>
        ...
        {sections.length === 0 && (
             <div className="text-center py-10 px-6 bg-white rounded-lg shadow-md">
                <p className="text-text-secondary">লাইব্রেরিতে কোনো শাখা পাওয়া যায়নি। শুরু করতে একটি নতুন শাখা যোগ করুন।</p>
             </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-40 flex justify-center items-center">
          <div className="bg-white p-8 rounded-lg shadow-2xl w-full max-w-md m-4">
            ...
            <form onSubmit={handleBookSubmit} className="space-y-4">
              <input type="text" name="title" value={bookToEdit ? bookToEdit.title : newBook.title} onChange={handleBookInputChange} placeholder="বইয়ের নাম" className="w-full px-4 py-2 border rounded-lg" required />
