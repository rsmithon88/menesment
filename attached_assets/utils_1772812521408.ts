...
export function numberToBengaliWords(num: number): string {
    ...
    let result = '';
    if (crore > 0) {
        // Recursive call to handle large numbers (e.g., thousands of crores)
...
export function printContent(elementId: string, documentTitle: string, extraCss: string = '') {
    ...
    if (!printContentEl) {
        ...
        alert('প্রিন্ট করার জন্য কোনো তথ্য পাওয়া যায়নি।');
        return;
    }

    const printWindow = window.open('', '', 'width=900,height=650');
    if (!printWindow) {
        alert('প্রিন্ট উইন্ডো খুলতে সমস্যা হয়েছে। অনুগ্রহ করে পপ-আপ ব্লকার চেক করুন।');
        return;
    }
