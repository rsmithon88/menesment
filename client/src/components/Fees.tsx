...
const Fees: React.FC<FeesProps> = ({ fees, students, addFee, updateFee, deleteFee, addMultipleFees, hasPermission, classOptions }) => {
  ...
  const [newRecord, setNewRecord] = useState({ studentId: '', amount: '', dueDate: '', status: 'অপরিশোধিত' as 'পরিশোধিত' | 'অপরিশোধিত' | 'বকেয়া', category: '' });
  ...
  const handleGenerateFees = (e: React.FormEvent) => {
    ...
    if (newFeesToGenerate.length > 0) {
      ...
      alert("এই শ্রেণীর সকল শিক্ষার্থীর জন্য এই মাসের ফি ইতিমধ্যে তৈরি করা হয়েছে।");
    }
    ...
    <div>
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6">
        ...
        <div className="flex items-center space-x-4">
            <div>
                <label htmlFor="classFilter" className="sr-only">শ্রেণী অনুযায়ী ফিল্টার</label>
                <select
                    id="classFilter"
                    value={filterClass}
        ...
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {filteredFees.map(record => {
              ...
              const isDue = record.status === 'অপরিশোধিত' || record.status === 'বকেয়া';
              return (
