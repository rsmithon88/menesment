...
const Results: React.FC<ResultsProps> = ({ results, students, exams, courses, addResult, updateResult, deleteResult, hasPermission, classOptions }) => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLSelectElement | HTMLInputElement>) => {
    ...
    stateSetter((prev: any) => (prev ? { ...prev, [name]: value } : null));
  };
  ...
  const addSubjectRow = () => {
    const stateSetter = isEditModalOpen ? setResultToEdit : setNewResult;
    stateSetter((prev: any) => (prev ? { ...prev, results: [...prev.results, { subject: '', marks: '', totalMarks: '100' }] } : null));
  };
  ...
  const removeSubjectRow = (index: number) => {
    ...
    stateSetter((prev: any) => {
        ...
        const updatedSubjects = prev.results.filter((_: any, i: number) => i !== index);
        return { ...prev, results: updatedSubjects };
    });
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const parsedResults: SubjectResult[] = newResult.results.map((r: any) => ({
        subject: r.subject,
    ...
    <div>
      ...
      <div className="mb-6 bg-white p-4 rounded-lg shadow-md no-print">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label htmlFor="classFilter" className="block text-sm font-medium text-gray-700">শ্রেণী অনুযায়ী ফিল্টার</label>
            ...
            <div>
                <label htmlFor="examFilter" className="block text-sm font-medium text-gray-700">পরীক্ষা অনুযায়ী ফিল্টার</label>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-40 flex justify-center items-center">
          <div className="bg-white p-8 rounded-lg shadow-2xl w-full max-w-2xl m-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <hr />
              <div>
                  <h4 className="text-lg font-medium mb-2">বিষয় ও নম্বর</h4>
                  {newResult.results.map((res: any, index: number) => (
                      <div key={index} className="grid grid-cols-12 gap-2 mb-2 items-center">
                          <input type="text" placeholder="বিষয়" value={res.subject} onChange={e => handleSubjectChange(index, 'subject', e.target.value)} className="col-span-5 w-full px-3 py-2 border rounded-lg" required />
                  ...
                  <button type="button" onClick={addSubjectRow} className="mt-2 text-sm text-primary hover:underline">+ আরও বিষয় যোগ করুন</button>
              </div>
